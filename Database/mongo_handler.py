...
# # Access a specific database (e.g., 'test' here)
# db = client.test

# # List all collections in the 'test' database
# print("Collections in 'test' database:", db.list_collection_names())
...
class MongoHandler:
    def __init__(self, connection_string: str, database_name: str):
        ...
        self.client = MongoClient(connection_string)
        self.db = self.client["my-database1"]  # Use database_name as specified in the argument
        
    def insert_posts(self, posts: List[Dict]):
        """Insert processed posts into MongoDB 'collection1' collection"""
        try:
            collection = self.db["collection1"]  # Use "collection1" as the collection name
